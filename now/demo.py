...
- 我需要对浦发银行，也就是股票代码为600000的股票进行分析。
"""
...
def read_data(is_all):
    is_all = is_all
    # 读取CSV文件并选择指定列
    df = pd.read_csv('./posts.csv', usecols=['id', 'stock_code', 'replay_num', 'post_title', 'read_num'])
    # 将数据按股票代码分组
...
def calculate_probability(sampled_groups,model):
    predicted = []
...
model = "ZHIPU"
if test_env == False:
    data = calculate_probability(read_data(is_all=True),model=model)
    df = pd.DataFrame(data)
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_path = f'./output_{current_time}.csv'
    df.to_csv(output_path, index=False)
else:
    calculate_probability(read_data(is_all=False),model=model)
