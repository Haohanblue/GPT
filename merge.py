...
Workflow:
- First step, Considering this message: {message} and additional
opinions from experts ,the mood agent:{expert_1},the rhetoric agent:{expert_2},the dependency agent:{expert_3},the aspect agent:{expert_4}and the reference agent:{expert_5}, what is the sentiment, positive/negative/neutral?
evaluate the sentiments distribution of the message,make the final overall evaluation of the sentiments (positive, negative, neutral) distribution.
...
df.head()
# 随机抽样50条数据，设置随机种子
sampled_data = df.sample(n=20, random_state=42)
predicted = []
labels = []
